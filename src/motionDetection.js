export class MotionDetection {
  constructor(options) {
    ...
    this.motionCallbackQueue = []
    this.options = options || {}

    this.diffCamEngine = diffCamEngineFactory()
  }
    ...
    return new Promise((resolve, reject) => {
      let options = Object.assign(this.options, {
        initSuccessCallback: () => {
          ...
          this.stream = this.diffCamEngine.getStream()
          resolve(this.stream)
        },
        ...
        captureCallback: (data) => {
          if (data.hasMotion) {
            this._triggerMotionCallbacks(data)
          }
        }
      })

      this.diffCamEngine.init(this.options)
    })
  }

  start() {
    this.diffCamEngine.start()
  }

  stop() {
    this.diffCamEngine.stop()
  }

  // when motion is detected
  onMotion(fn) {
    this._log('motion detected')
    this.motionCallbackQueue.push(fn)
  }

  _triggerMotionCallbacks(data) {
    this.motionCallbackQueue.forEach(fn => fn(data))
  }

  _log(...args) {
     console.debug('[motion detection]', ...args)
  }
}

// adapted from https://github.com/lonekorean/diff-cam-engine
function diffCamEngineFactory() {
  ...
  var coords;
	...
	function initSuccess(requestedStream) {
		...
		// initSuccessinitSuccessCallbackCallback();
    initSuccessCallback();
	}
